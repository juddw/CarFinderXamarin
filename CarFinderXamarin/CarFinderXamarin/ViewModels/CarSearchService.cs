...
using CarFinderXamarin.Services;
...
namespace CarFinderXamarin.ViewModels
{
    public class CarService
    {
        const string carUrl = "https://jwhitehead-carapi.azurewebsites.net/api/Cars/";
        HttpClient client = new HttpClient();

        public async Task<List<string>> GetYears()
        {
            var json = await client.GetStringAsync(carUrl + "Year");
            var returnedData = JsonConvert.DeserializeObject<List<string>>(json);
            return returnedData;
        }
        ...
        {
            string _year_make = string.Format("?Year={0}&Make={1}", year, make); // year is 0 index, make is 1 index
            var json = await client.GetStringAsync(carUrl + "Model" + _year_make);
            var returnedData = JsonConvert.DeserializeObject<List<string>>(json);
            return returnedData;
            //string nhtsaModels = string.Format("https://one.nhtsa.gov/webapi/api/Recalls/vehicle/modelyear{0}/make/{1}?fromat=jsaon", year, make);
        }
